...
static void ciri_stop(int sig) {
  log_info(logger_id, "signal handler, sig=%d\n", sig);
...
int dag_main(int argc, char* argv[]) {
  ...
  if (iota_ciri_conf_default_init(&ciri_core.conf, &ciri_core.consensus.conf, &ciri_core.node.conf, &api.conf) !=
      RC_OK) {
    log_critical(logger_id, "Initializing conf default failed\n");
    return EXIT_FAILURE;
  }
  ...
  if (iota_ciri_conf_file_init(&ciri_core.conf, &ciri_core.consensus.conf, &ciri_core.node.conf, &api.conf, argc,
                               argv) != RC_OK) {
    log_critical(logger_id, "Initializing conf file failed\n");
    return EXIT_FAILURE;
  }
  ...
  if (iota_ciri_conf_cli_init(&ciri_core.conf, &ciri_core.consensus.conf, &ciri_core.node.conf, &api.conf, argc,
                              argv) != RC_OK) {
    log_critical(logger_id, "Initializing conf cli failed\n");
    return EXIT_FAILURE;
  }
...
/************
 *
 * ./bazel-bin/ciri/ciri  -n tcp://192.168.1.83:15600 -p 24265  --log-level debug
 *
 ************/
int dag_start(void) {
    int argc;
...
void dag_stop(){
  log_info(logger_id, "dag stopped ...\n");
  if (core_stop(&ciri_core) != RC_OK) {
      log_error(logger_id, "Stopping cIRI core failed\n");
  }
}
