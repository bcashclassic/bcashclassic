...
retcode_t uint64_t_to_flex_trit_t_map_add(uint64_t_to_flex_trit_t_map_t *const map,
  ...
  uint64_t_to_flex_trit_t_map_entry_t *map_entry = NULL;
  map_entry = (uint64_t_to_flex_trit_t_map_entry_t *)malloc(
      sizeof(uint64_t_to_flex_trit_t_map_entry_t));

  if (map_entry == NULL) {
    return RC_OOM;
  }

  if ((map_entry->key = (uint64_t*)malloc(map->key_size)) == NULL) {
    return RC_OOM;
  }

  if ((map_entry->value = (flex_trit_t*)malloc(map->value_size)) == NULL) {
    return RC_OOM;
  }
...
retcode_t uint64_t_to_flex_trit_t_map_free(uint64_t_to_flex_trit_t_map_t *const map) {
  uint64_t_to_flex_trit_t_map_entry_t *curr_entry = NULL;
  uint64_t_to_flex_trit_t_map_entry_t *tmp_entry = NULL;
...
uint64_t_to_flex_trit_t_map_t const *const rhs){
  ...
  if (!uint64_t_to_flex_trit_t_map_contains(rhs,curr_entry->key)){
    return false;
    }
  }
  return true;
...
uint64_t_to_flex_trit_t_map_entry_t *entry = NULL;
  ...
  uint64_t_to_flex_trit_t_map_remove_entry(map, entry);
