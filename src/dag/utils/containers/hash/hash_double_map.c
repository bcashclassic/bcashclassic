...
retcode_t hash_to_double_map_add(hash_to_double_map_t *const map,
                                  ...
                                  double value) {
  hash_to_double_map_entry_t *map_entry = NULL;

  HASH_FIND(hh, *map, hash, FLEX_TRIT_SIZE_243, map_entry);
  if (map_entry == NULL){
    map_entry = (hash_to_double_map_entry_t *)malloc(
    sizeof(hash_to_double_map_entry_t));
...
bool hash_to_double_map_contains(hash_to_double_map_t const *const map,
                                  flex_trit_t const *const hash) {
  hash_to_double_map_entry_t *entry = NULL;
...
bool hash_to_double_map_remove(hash_to_double_map_t *const map,
        flex_trit_t const *const hash){
  hash_to_double_map_entry_t *entry = NULL;
...
void hash_to_double_map_free(hash_to_double_map_t *const map) {
  hash_to_double_map_entry_t *curr_entry = NULL;
  hash_to_double_map_entry_t *tmp_entry = NULL;
...
void hash_to_double_map_keys(hash_to_double_map_t *const map, hash243_set_t * const keys){
  hash_to_double_map_entry_t *curr_entry = NULL;
  hash_to_double_map_entry_t *tmp_entry = NULL;
  HASH_ITER(hh, *map, curr_entry, tmp_entry) {
...
retcode_t hash_to_double_map_copy(hash_to_double_map_t const *const src,
hash_to_double_map_t *const dst){
  ...
  HASH_ITER(hh, *src, curr_entry, tmp_entry) {
    ret = hash_to_double_map_add(dst,curr_entry->hash, curr_entry->value);
    if (ret != RC_OK){
      return ret;
    }
  }
...
size_t hash_to_double_map_size(hash_to_double_map_t const map){
 return HASH_COUNT(map);
}


bool hash_to_double_map_equal(hash_to_double_map_t const lhs,
        hash_to_double_map_t const rhs){
  if (HASH_COUNT(lhs) != HASH_COUNT(rhs)){
    return false;
  }

  hash_to_double_map_entry_t * rhs_entry = NULL;
  hash_to_double_map_entry_t *lhs_entry = NULL;
  hash_to_double_map_entry_t *tmp_entry = NULL;

  HASH_ITER(hh, lhs, lhs_entry, tmp_entry) {
    if (!hash_to_double_map_find(&rhs,lhs_entry->hash,&rhs_entry)){
      return false;
    }
