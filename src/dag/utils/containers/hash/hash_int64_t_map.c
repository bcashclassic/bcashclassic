...
retcode_t hash_to_int64_t_map_add(hash_to_int64_t_map_t *const map,
  ...
  if (map_entry == NULL){
    map_entry = (hash_to_int64_t_map_entry_t *)malloc(
    sizeof(hash_to_int64_t_map_entry_t));
...
bool hash_to_int64_t_map_contains(hash_to_int64_t_map_t const *const map,
                                  flex_trit_t const *const hash) {
  hash_to_int64_t_map_entry_t *entry = NULL;
...
bool hash_to_int64_t_map_find(hash_to_int64_t_map_t const *const map,
                              flex_trit_t const *const hash,
                              hash_to_int64_t_map_entry_t ** const res) {
  if (map == NULL || *map == NULL) {
    return false;
  }
  if (res == NULL) {
...
void hash_to_int64_t_map_free(hash_to_int64_t_map_t *const map) {
  hash_to_int64_t_map_entry_t *curr_entry = NULL;
  hash_to_int64_t_map_entry_t *tmp_entry = NULL;
...
void hash_to_int64_t_map_keys(hash_to_int64_t_map_t *const map, hash243_set_t * const keys){
  hash_to_int64_t_map_entry_t *curr_entry = NULL;
  hash_to_int64_t_map_entry_t *tmp_entry = NULL;
  HASH_ITER(hh, *map, curr_entry, tmp_entry) {
...
hash_to_int64_t_map_t *const dst){
  ...
  HASH_ITER(hh, *src, curr_entry, tmp_entry) {
    ret = hash_to_int64_t_map_add(dst,curr_entry->hash, curr_entry->value);
    if (ret != RC_OK){
      return ret;
    }
  }
...
size_t hash_to_int64_t_map_size(hash_to_int64_t_map_t const map){
...
bool hash_to_int64_t_map_equal(hash_to_int64_t_map_t const lhs,
  ...
  hash_to_int64_t_map_entry_t * rhs_entry = NULL;
  hash_to_int64_t_map_entry_t *lhs_entry = NULL;
  hash_to_int64_t_map_entry_t *tmp_entry = NULL;

  HASH_ITER(hh, lhs, lhs_entry, tmp_entry) {
    if (!hash_to_int64_t_map_find(&rhs,lhs_entry->hash,&rhs_entry)){
      return false;
    }
